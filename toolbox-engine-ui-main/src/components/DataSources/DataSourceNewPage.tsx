...
const DataSourceNewPage: React.FC<Props> = ({ dataSource = null }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      showAlert("success", "Data source added successfully!");
  ...
  const upsertDataSource = async () => {
    let response;
    if (dataSource) {
      response = await toolboxEngineAPI.put(`/api/data_sources/${dataSource.id}`, {
        name,
        url
      });
    } else {
      response = await toolboxEngineAPI.post("/api/data_sources", {
        name,
        url
      });
    }
          ...
          >
            <div className="flex items-center justify-between mb-[20px]">
              <h5 className="font-semibold mb-[20px] text-[#1f1f1f] font-['Segoe_UI',sans-serif]">
                {dataSource ? "Edit data source" : "Add a new data source"}
              </h5>
              <button
                type="button"
