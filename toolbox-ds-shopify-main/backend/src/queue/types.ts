...
export const getEventFromJob = (job: Job): any | undefined => {
    ...
    switch (job.name) {
        ...
        case 'push.count':
            return new PushCountEvent(job.data.dataSourceId, job.data.pullId, job.data.count);
        case 'push.entity':
            return new PushEntityEvent(job.data.dataSourceId, job.data.pullId, job.data.entity);
        case 'pull.completed':
            return new PullCompletedEvent(job.data.dataSourceId, job.data.pullId);
        default:
